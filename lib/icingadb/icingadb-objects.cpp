		...
		std::map<String, std::vector<String>>& publishes, bool runtimeUpdate)
{
	...
	if (customVarObject) {
		...
		if (vars) {
			...
			for (auto& kv : vars) {
				...
				String id = HashValue(new Array(Prepend(env, kv.first, GetObjectIdentifiersPerEnv(object))));
	...
	if (type == Host::TypeInstance || type == Service::TypeInstance) {
		...
		if (groups) {
			...
			for (auto& group : groups) {
				...
				String id = HashValue(new Array(Prepend(env, GetObjectIdentifiersPerEnv(groupObj), GetObjectIdentifiersPerEnv(object))));
	...
	if (type == TimePeriod::TypeInstance) {
		...
		if (ranges) {
			...
			for (auto& kv : ranges) {
				...
				String id = HashValue(new Array(Prepend(env, kv.first, kv.second, GetObjectIdentifiersPerEnv(object))));
		...
		for (auto include : includes) {
			...
			String id = HashValue(new Array(Prepend(env, GetObjectIdentifiersPerEnv(includeTp), GetObjectIdentifiersPerEnv(object))));
		...
		for (auto exclude : excludes) {
			...
			String id = HashValue(new Array(Prepend(env, GetObjectIdentifiersPerEnv(excludeTp), GetObjectIdentifiersPerEnv(object))));
	...
	if (type == Zone::TypeInstance) {
		...
		for (auto& parent : parentsRaw) {
			String id = HashValue(new Array(Prepend(env, GetObjectIdentifiersPerEnv(parent), GetObjectIdentifiersPerEnv(object))));
	...
	if (type == Notification::TypeInstance) {
		...
		for (auto& user : users) {
			...
			String id = HashValue(new Array(Prepend(env, GetObjectIdentifiersPerEnv(user), GetObjectIdentifiersPerEnv(object))));
		...
		for (auto& usergroup : usergroups) {
			...
			String id = HashValue(new Array(Prepend(env, "usergroup", GetObjectIdentifiersPerEnv(usergroup), GetObjectIdentifiersPerEnv(object))));
		...
		for (auto& user : allUsers) {
			String userId = GetObjectIdentifier(user);
			String id = HashValue(new Array(Prepend(env, "user", GetObjectIdentifiersPerEnv(user), GetObjectIdentifiersPerEnv(object))));
			notificationRecipients.emplace_back(id);
